...
def get_trans_matrix(points):
    ...
    for conf_ind in range(1, 13):
        ...
        if all(point is not None for point in inters):
            ...
            for i in range(12):
                if i not in inds and points[i] is not None:
                    # Ensure that points[i] and trans_kps[i] are 1-D
                    point_1d = np.array(points[i]).flatten()
                    trans_kp_1d = np.array(trans_kps[i]).flatten()
                    dists.append(distance.euclidean(point_1d, trans_kp_1d))
