...
class CourtReference:
    ...
    def get_court_mask(self, mask_type=0):
        ...
        if mask_type == 1:  # Bottom half court 仅保留场地的下半部分
            # mask[:self.net[0][1] - 1000, :] = 0
            mask[:self.net[0][1], :] = 0
        elif mask_type == 2:  # Top half court 仅保留场地的上半部分
            mask[self.net[0][1]:, :] = 0
        elif mask_type == 3: # court without margins 去除场地的边界部分
